...
void matrixAreas::setup(float x, float y, float w, float h, int _nAreas){
    ...
    automatFont.load("D3Litebitmapism.ttf",8, true, true);
...
void matrixAreas::draw(){
    ...
    for (int i = 0; i < numAreas; i++){
        ...
        ofDrawCircle(xCalib + points[i].x*scalew, yCalib + points[i].y*scaleh, 5);
        ...
        ofDrawCircle(xCalib + points[i].x*scalew, yCalib + points[i].y*scaleh, diameters[i]);
    }
    glDisable(GL_BLEND);
...
void matrixAreas::drawMinimalWithLines(){
    ...
    ofDrawLine(xCalib + points[0].x*scalew, yCalib + points[0].y*scaleh,xCalib + points[1].x*scalew, yCalib + points[1].y*scaleh);
    ofDrawLine(xCalib + points[1].x*scalew, yCalib + points[1].y*scaleh,xCalib + points[2].x*scalew, yCalib + points[2].y*scaleh);
    ofDrawLine(xCalib + points[2].x*scalew, yCalib + points[2].y*scaleh,xCalib + points[3].x*scalew, yCalib + points[3].y*scaleh);
    ofDrawLine(xCalib + points[3].x*scalew, yCalib + points[3].y*scaleh,xCalib + points[0].x*scalew, yCalib + points[0].y*scaleh);

    ofDrawLine(xCalib + points[0].x*scalew, yCalib + points[0].y*scaleh,xCalib + points[2].x*scalew, yCalib + points[2].y*scaleh);
    ofDrawLine(xCalib + points[1].x*scalew, yCalib + points[1].y*scaleh,xCalib + points[3].x*scalew, yCalib + points[3].y*scaleh);
...
void matrixAreas::drawMinimalGrid(){
    ...
    for (int i = 0; i < numAreas; i++){
        if(pointBeingCalibrated == i && imOver[i] == true){
            glColor4f(1.0f,0.906f,0.463f, 1.0f);
            ofDrawRectangle(xCalib + points[i].x*scalew - 15, yCalib + points[i].y*scaleh - 15, 30,30);
            ofDrawRectangle(xCalib + points[i].x*scalew - 40, yCalib + points[i].y*scaleh - 40, 80,80);
        }else if(imOver[i] == true){
            glColor4f(1.0f,0.906f,0.463f, 1.0f);
            ofDrawRectangle(xCalib + points[i].x*scalew - 5, yCalib + points[i].y*scaleh - 5, 10,10);
            ofDrawRectangle(xCalib + points[i].x*scalew - 10, yCalib + points[i].y*scaleh - 10, 20,20);
            ofDrawRectangle(xCalib + points[i].x*scalew - 15, yCalib + points[i].y*scaleh - 15, 30,30);
        }else{
            ofSetColor(0,255,0);
            ofDrawRectangle(xCalib + points[i].x*scalew - 5, yCalib + points[i].y*scaleh - 5, 10,10);
